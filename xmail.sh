...
#
# SP (FW,MR), Stand August 2013
#
#
# Aenderungen:
# Auswahlfenster fuer Email an Patient, H-Arzt oder U-Arzt
...
# Emailadresse
# Patient
# Emailfeld u. die 4 n�chsten Felder auf Mailadresse untersuchen
# wenn Emailfeld Eintrag ohne @-Zeichen enth�lt wir Emailadresse auf "" gesetzt (keine Adresse)
...
# wer wurde ausgew�hlt? (Pat, H-Arzt oder U-Arzt)
...
case $wer in
   ...
   H) ges1=`sed -n '2p' $file | awk -F";" -v POS=$hgespos '{print $POS}' | sed 's/\"//g'`;
      # 1. Zeichen (m aus m�nnlich oder w aus weiblich)
      ges=`echo ${ges1:0:1}`;
      name=$hname;;
